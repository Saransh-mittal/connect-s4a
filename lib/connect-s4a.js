  ...
  return function (req, res, next) {
    ...
    function serveCapture() {
      ...
      proxiedReq.on('response', proxiedRes => {
        console.log(
          `[SEO4Ajax] ${proxiedRes.statusCode} response for ${req.url}`,
        ) // Log status + URL

        if (proxiedRes.statusCode === 503 || proxiedRes.statusCode === 404) {
          console.log(
            `[SEO4Ajax] Cache miss for ${req.url}. Triggering background caching.`,
          )
          next()
        } else {
          console.log(`[SEO4Ajax] Serving cached version of ${req.url}`)
          proxiedRes.pipe(res).on('error', next)
        }
      })

      req.pipe(proxiedReq).on('error', next)
    }
